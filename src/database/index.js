...
const validServerSettingsColumns = [
    'setup_completed', 'mod_role_id', 'disabled_commands', 'log_channel_id', 'reports_channel_id', 
    'warning_threshold', 'warning_expire_days', 'cooldown_seconds', 'rules_channel_id',
    'spam_protection', 'spam_threshold', 'spam_interval', 'spam_warning_message', 
    'channel_restrictions_enabled', 'content_filter_enabled', 'content_filter_log_suspicious', 
    'content_filter_notify_user', 'content_filter_notify_message', 'tickets_channel_id', 
    'tickets_category_id', 'tickets_enabled', 'created_at', 'updated_at'
];
...
async function initDatabase() {
   try {
       ...
       await db.run(`
        ...
        await db.run(`
            CREATE TABLE IF NOT EXISTS active_timeouts (
                ...
                reason TEXT DEFAULT 'Auto-timeout',
                PRIMARY KEY (guild_id, user_id)
            )
        `);
        ...
        await db.run(`
            CREATE TABLE IF NOT EXISTS long_cooldowns (
                guild_id TEXT NOT NULL,
                user_id TEXT NOT NULL,
       ...
       await db.run(`CREATE INDEX IF NOT EXISTS idx_user_support_user ON user_support(user_id)`);
       await db.run(`CREATE INDEX IF NOT EXISTS idx_user_support_sku ON user_support(sku_id)`);
       ...
       await db.run(`CREATE INDEX IF NOT EXISTS idx_timeouts_expires ON active_timeouts(expires_at)`);
       await db.run(`CREATE INDEX IF NOT EXISTS idx_countdowns_end ON active_countdowns(end_time)`);
       await db.run(`CREATE INDEX IF NOT EXISTS idx_cooldowns_expires ON long_cooldowns(expires_at)`);
...
const database = {
  ...
  clearExpiredWarnings: async () => {
   ...
   getUserReminders: async (userId) => {
        try {
            ...
            return await db.all(`
                SELECT id, message, reminder_time, created_at
                FROM reminders
  ...
  getCurrentBOTD: async () => {
   ...
   logSupport: async (userId, skuId, isSubscription = false) => {
        try {
            if (!validateUserId(userId) || !validateString(skuId, 100)) {
                return { error: 'Invalid parameters' };
            }
            
            return await db.run(`
                INSERT INTO user_support (user_id, sku_id, is_subscription, created_at)
                VALUES (?, ?, ?, CURRENT_TIMESTAMP)
            `, [userId, skuId, isSubscription ? 1 : 0]);
        } catch (error) {
            console.error('Error logging support:', error);
            return { error: 'Database error' };
        }
    },

    // Get subscription renewal count
    getSubscriptionRenewals: async (userId, skuId) => {
        try {
            const result = await db.get(`
                SELECT COUNT(*) as count 
                FROM user_support 
                WHERE user_id = ? AND sku_id = ? AND is_subscription = 1
            `, [userId, skuId]);
            
            return result.count || 0;
        } catch (error) {
            console.error('Error getting subscription renewals:', error);
            return 0;
        }
    },
        ...
        try {
            return await db.all(`
                SELECT 
                    ...
                    MIN(created_at) as first_support,
                    MAX(created_at) as latest_support
                FROM user_support 
                GROUP BY user_id, sku_id
                ORDER BY latest_support DESC
            `);
        } catch (error) {
   ...
   // === TIMEOUT METHODS ===
    saveActiveTimeout: async (guildId, userId, expiresAt, reason = 'Auto-timeout') => {
        try {
            return await db.run(`
                INSERT OR REPLACE INTO active_timeouts (guild_id, user_id, expires_at, reason)
                VALUES (?, ?, ?, ?)
            `, [guildId, userId, expiresAt.toISOString(), reason]);
        } catch (error) {
            console.error('Error saving active timeout:', error);
            return { error: 'Database error' };
        }
    },

    getActiveTimeouts: async () => {
        try {
            return await db.all(`
    ...
    removeActiveTimeout: async (guildId, userId) => {
        try {
            return await db.run(`
            ...
            console.error('Error removing active timeout:', error);
            return { error: 'Database error' };
        }
    },

    cleanupExpiredTimeouts: async () => {
        try {
            return await db.run(`
                DELETE FROM active_timeouts 
                WHERE expires_at <= datetime('now')
    ...
    createCountdown: async (countdownData) => {
        try {
            return await db.run(`
                INSERT INTO active_countdowns 
                (guild_id, channel_id, message_id, title, completion_message, total_seconds, end_time, created_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `, [
                countdownData.guild_id,
                countdownData.channel_id,
                countdownData.message_id,
                countdownData.title,
                countdownData.completion_message,
                countdownData.total_seconds,
                countdownData.end_time.toISOString(),
                countdownData.created_by
            ]);
        } catch (error) {
    ...
    getActiveCountdowns: async () => {
        try {
            return await db.all(`
                SELECT * FROM active_countdowns
                WHERE end_time > datetime('now')
                ORDER BY end_time ASC
            `);
        } catch (error) {
            console.error('Error getting active countdowns:', error);
            return [];
        }
    },

    deleteCountdown: async (messageId) => {
        try {
            return await db.run('DELETE FROM active_countdowns WHERE message_id = ?', [messageId]);
        } catch (error) {
    ...
    // === COOLDOWN METHODS ===
    saveLongCooldown: async (guildId, userId, commandName, expiresAt) => {
        try {
            return await db.run(`
                INSERT OR REPLACE INTO long_cooldowns (guild_id, user_id, command_name, expires_at)
                VALUES (?, ?, ?, ?)
            `, [guildId, userId, commandName, expiresAt.toISOString()]);
        } catch (error) {
            console.error('Error saving long cooldown:', error);
            return { error: 'Database error' };
        }
    },

    getLongCooldowns: async () => {
        try {
            return await db.all(`
                SELECT * FROM long_cooldowns
                WHERE expires_at > datetime('now')
            `);
        } catch (error) {
            console.error('Error getting long cooldowns:', error);
            return [];
        }
    },

    removeLongCooldown: async (guildId, userId, commandName) => {
        try {
            return await db.run(`
                DELETE FROM long_cooldowns 
                WHERE guild_id = ? AND user_id = ? AND command_name = ?
            `, [guildId, userId, commandName]);
        } catch (error) {
            console.error('Error removing long cooldown:', error);
            return { error: 'Database error' };
        }
    },

    cleanupExpiredCooldowns: async () => {
        try {
            return await db.run(`
                DELETE FROM long_cooldowns 
                WHERE expires_at <= datetime('now')
