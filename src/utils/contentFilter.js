...
async function handleViolation(message, term, shouldDelete = true, violationType = 'EXPLICIT') {
   try {
       ...
       if (shouldDelete && botMember.permissions.has('ManageMessages')) {
           try {
               // Track this deletion
               trackBotAction('message_delete', message.guildId, message.author.id, {
                   reason: `Content filter: ${violationType}`
               });
       ...
       try {
           if (punishmentType === 'MUTE') {
               // Track this timeout BEFORE applying it
               trackBotAction('timeout', message.guildId, message.author.id, {
                   reason: `Content filter: ${violationType}`,
                   duration: punishmentDuration
               });
               
               await member.timeout(punishmentDuration, `Content filter: Posted ${violationType.toLowerCase()} content (Warning ${warningCount}/${warningThreshold})`);

               if (message.client.timeoutManager) {
                   const expiresAt = new Date(Date.now() + punishmentDuration);
                   await message.client.timeoutManager.addTimeout(
                       message.guildId, 
                       message.author.id, 
                       expiresAt, 
                       `Content filter: ${violationType}`,
                       message.author.tag
                   );
               }
               ...
               // Track this ban BEFORE applying it
               trackBotAction('ban', message.guildId, message.author.id, {
                   reason: `Content filter: ${violationType}`
               });

               // Then proceed with ban
               await member.ban({
                    reason: `Content filter: Exceeded warning threshold (${warningThreshold}/${warningThreshold}) for ${violationType.toLowerCase()} content`,
